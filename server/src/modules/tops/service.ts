...
async function assertOwner(userId: string, topId: string) {
...
async function assertItemAllowed(userId: string, animeId: string) {
  ...
  if (!w || w.status !== 'WATCHED' || w.rating == null) {
    throw new HttpError(400, 'В топ можно добавлять только «Просмотрено» с вашей оценкой');
  }
}
...
async function loadItems(topIds: string[]) {
  ...
  return db.select({
    ...
    .leftJoin(watchItems, and(eq(watchItems.animeId, animeTable.id), eq(watchItems.userId, userTops.userId)))
...
export async function getTop(topId: string) {
  const [top] = await db.select(topSelect)
    .from(userTops)
    .innerJoin(users, eq(userTops.userId, users.id))
    .where(eq(userTops.id, topId)).limit(1);
  if (!top) throw new HttpError(404, 'Top not found');
...
export async function createTop(userId: string, input: unknown) {
  ...
  return db.transaction(async (tx) => {
    const [top] = await tx.insert(userTops).values({
      userId, name: p.name, description: p.description ?? null, contentType: p.contentType,
    }).returning();
    let pos = 1;
    for (const it of p.items ?? []) {
      await tx.insert(userTopItems).values({ topId: top.id, animeId: it.animeId, position: pos++, comment: it.comment ?? null });
    }
    return { id: top.id };
  });
}
...
export async function removeTop(userId: string, topId: string) {
  await assertOwner(userId, topId);
...
export async function addTopItem(userId: string, topId: string, input: unknown) {
  ...
  const existing = await db.select({ position: userTopItems.position }).from(userTopItems).where(eq(userTopItems.topId, topId));
  const position = existing.reduce((m, i) => Math.max(m, i.position), 0) + 1;
  try {
    await db.insert(userTopItems).values({ topId, animeId: parsed.data.animeId, position, comment: parsed.data.comment ?? null });
