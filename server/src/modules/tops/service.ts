...
export async function listTops(userId: string) {
  ...
  const items = await db
    .select({
      id: userTopItems.id, topId: userTopItems.topId, position: userTopItems.position,
      animeId: animeTable.id, shikimoriId: animeTable.shikimoriId,
      source: animeTable.source, contentType: animeTable.contentType,
      name: animeTable.name, russian: animeTable.russian,
      coverImage: animeTable.coverImage, score: animeTable.score,
    })
    .from(userTopItems)
